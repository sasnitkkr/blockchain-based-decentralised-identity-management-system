...
contract Defence{
    ...
    function addEmployees(Employee[] memory employee) public restricted{
        ...
        for(i=0; i<employee.length; i++){
            ...
            bytes memory empRank = bytes(employee[i].empRank);
    ...
    function verifyEmployee(Employee memory employee) public view returns (Employee){
        bytes32 encryptedAdmin = sha256(abi.encodePacked(admin));
        bytes memory empId = bytes(employee[i].empId);
        bytes memory empRank = bytes(employee[i].empRank);
        bytes memory ihash = abi.encodePacked(empId,empRank);
    ...
    function removeEmployee(EmployeeData memory student) public restricted{
        bytes32 encryptedAdmin = sha256(abi.encodePacked(admin));
        bytes memory empId = bytes(employee[i].empId);
        bytes memory empRank = bytes(employee[i].empRank);
        bytes memory ihash = abi.encodePacked(empId,empRank);
